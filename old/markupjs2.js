...
const c3Linker = function c3Linker(input, tags) {
  const _scafoldRegularTaggedTemplate = function _scafoldRegularTaggedTemplate() {
    function _regularTaggedTemplate(literals, ...substitutions) {
      ...
      let output = _taggedTemplate(keyword, literals, substitutions);
      output += `</${keyword}>`;
      _document += `</${keyword}>`;
      return output;
    }
  ...
  const _scafoldIrregularTaggedTemplate = function _scafoldIrregularTaggedTemplate() {
    function _irregularTaggedTemplate(literals, ...substitutions) {
      let keyword = '_regularTaggedTemplate';
      let output = _taggedTemplate(keyword, literals, substitutions);
      output += `</${keyword}>`;
      _document += `</${keyword}>`;
  ...
  const _scafoldTaggedTemplate = function _scafoldTaggedTemplate() {  
    function _taggedTemplate(keyword, literals, substitutions) {
      let output = `<${keyword}`;
      let attributes = {};
  ...
  output += /(?:function _scafoldTaggedTemplate\(\) {)([^]*)}/
    .exec(_scafoldTaggedTemplate.prototype.constructor)[1] // Exclude the first and last lines of "_scafold.prototype.constructor".
    .replace(/^([^\S(?:(?:\r\n?)|\n|\u2028|\u2029))]){4}/mg, ''); // Remove the four spaces at each line beginning ([^\S\n] = "\s and not \n").
...
const c4Loader = function c4Loader(input, tags) {
  const _scafold = function _scafold() {
    ...
    _template += _taggedTemplate.prototype.constructor + '\n';
