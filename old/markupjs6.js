...
const c2Resolver = function c2Resolver(template, input) {
  ...
  const tagNode = function tagNode(node) {
    ...
    if (html5.regularTags.indexOf(tag) !== -1 && tags[0].indexOf(tag) === -1)
      tags[0].push(tag);
    else if (html5.irregularTags.indexOf(tag) !== -1 && tags[1].indexOf(tag) === -1) 
      tags[1].push(tag);
    else if (html5.specialTags.indexOf(tag) !== -1 && tags[2].indexOf(tag) === -1) 
      tags[2].push(tag);  
    if (tag === 'custom_tags') {
...
const c3Linker = function c3Linker(input, tags) {
  ...
  const _scafoldParseContent = function _scafoldParseContent() {
    function _parseContent(literals, substitutions, isRaw = false) {
      ...
      for (; i < len; i++) {
        output += isRaw ? literals.raw[i] : literals[i]//.replace(//g,'');
