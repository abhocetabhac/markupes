...
// following "node bin/markupes" in the command-line .
const argv = process.argv.slice(2);

let usage = `  Usage:
    markupes [options] path/to/template.js`;
let switches = [
  ['-v', '--version', 'display markupES version.'],
  ...
  ['-d', '--debug', 'displays debug information ("cli.js", "markupes.js").'],
  ['-D', '--Debug', 'displays further debug information ("thud-clunk.js").']
];
...
this.run = () => {
  ...
  let o = this.options;
  if (o.debug) { markupes.debug(true); debug(true); }
  if (o.Debug) { thunkify.debug(true); }
  de&&bug(BLU+'Command-line options = '+RST, o);
  if (o.version) { console.log('markupES v.', markupes.version); }
  ...
  if (o.utils) {
    ...
    o.locals.render = (file) => { // this === module.exports
      ...
      return markupes.render(contents, this.options); 
    };
  }
...
const fnCompile = (err = null, input = null, args) => {
  ...
  let output;
  try {
    output = markupes.compile(input, this.options);
  } catch (err) {
    process.exitCode = 4;
  ...
  if (this.options.autoexec) {
    output = '#!/usr/bin/env node\n' +
      'const debug = !!process.argv.slice(2).length;\n' +
      '(function(debug) {\n' + 
      output.replace(/(?:return\srun\.bind\(metadata\);)[^]*?$/, 
        String.raw`debug && console.log('\x1b[36m_template:\x1b[0m' + _template + '\x1b[36m' + '-'.repeat(80) + '\x1b[0m');`
        + `\nconsole.log(run.call(metadata));\n})(debug);`);
...
const fnRender = (err = null, input = null, args) => {
  ...
  let output;
  try {
    output = markupes.render(input, this.options);
  } catch (err) {
    process.exitCode = 5;
...
//     let name = path.basename(inPath, path.extname(inPath));
//     if (!js) {
//       output = markupes.render(contents, this.options);
//       ext = '.html';
//     } else {
//       let func = markupes.compile(contents, this.options);
//       output = `(function() {
    ...
    const appendTemplate = () => {
      ...
      if (i >= inPath.length || !(this.options.package)) {
        ...
        return fs.readFile(inPath[i], 'utf-8', function(err, contents) {
          ...
          func = markupes.compile(contents, this.options);
